...
class AnalogSensor(Sensor):
    def __init__(self,hexAddress,channelNumber):
        ...
        :param hexAddress:  the I2C address of the device
        :type hexAddress: hex / integer
        :param channelNumber: the MMS channel # the device is connected to
        :type channelNumber: integer, indexed from 0
        :return: an object representing the analog sensor object
        :rtype: AnalogSensor
        """
        ...
        self._internalDevice = ADS1x15(address=hexAddress)
        self.sensorName = 'Analog Sensor V1.01'
        self.sensorReadingType = 'VOLTAGE'
        self.plotType = PlotType.TimeSeriesMulti
        self.gain = 4096
        self.sps = 250
        self.milliToVolts = 1000
        self.channels = 4
    def getData(self):
        """
        Gets the sensors current state and returns a
        dict / json object containing the value,timestamp,typing and id of the sensor's curent state
        :return: returns a dict / json object containing the value,timestamp,typing and id of the sensor's curent state
        :rtype: dict
        """
        value = self.getValue()
    ...
    def getValue(self):
        """
        Reads the analog sensor's ADC and parses results into an array (for channels on ADC, channels indexed from 0)
        :return: array representing each analog channels value
        :rtype: Array/List
        """
        value = []
        for i in range(0,self.channels):
            value.append(self._internalDevice.readADCSingleEnded(i,self.gain,self.sps) / 1000)
        return value
    def getMetaDataInfo(self):
        """
        gets the metadata for the sensor
        :return: metadata about the sensorobj, including id,sensorName,sensorReadingType,plotType,
        lastSyncTime,and signal count
        :rtype: dict/json
        """
        return {'_id':self.channelNumber,'sensorName':self.sensorName,
                'sensorReadingType':self.sensorReadingType,
                'plotType':self.plotType,'lastSyncTime':time.time(),'signals':self.channels}
    def isRational(self):
        """
        Determines if all values in the array are the same or not
        :return: true if all values in the array are not the same
        :rtype: bool
        """
        #determines if a valid analog sensor or different sensor with the same address
        val = self.getValue()
        if(val[1:] == val[:-1]):
            return False
        return True
